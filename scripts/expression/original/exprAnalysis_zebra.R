...
resdir <- file.path(maindir,"results/zebra_UvC")
...
gtffile <- file.path(maindir,"resources/Danio_rerio.GRCz10.83.protcoding.gtf")
...
print("Warning: Using Unaffected as primary status")
se$Status <- relevel(se$Status, "U")
...
## ------------------------------------------------------------------------
resGR$symbol <- mapIds(org.Dr.eg.db, names(resGR), "SYMBOL", "ENSEMBL")
