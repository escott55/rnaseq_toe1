...
cntDir <- file.path(basedir,"raw/zebrafish/exoncnt")
#flattenedFile <- file.path(basedir,"resources/Homo_sapiens.GRCh37.b37.exons.gff")
flattenedFile <- file.path(basedir,"resources/Danio_rerio.GRCz10.83.exons.gff")
figDir <- file.path(basedir, "exonusage/zebra")

csvfile <- file.path(basedir,"raw/zebrafish/SampleTable.txt")
...
figname <- file.path(figDir,"toe1_ma.pdf")
pdf(figname)
plotMA( dxr1, cex=0.8 )
dev.off()
...
sigset <- dxr1filt[ (dxr1filt$padj < 0.05) ,]
